"""Export MLflow traces for the hkia_baseline experiment runs as CSV.

One CSV per run, written to data/eval/traces/. The script targets the
five run names captured in RUN_NAMES below; missing runs are warned
about but do not abort the export of the remaining runs.
"""
...
# Allow `python scripts/export_traces.py` to import the project's top-level modules.
PROJECT_ROOT = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(PROJECT_ROOT))

from config.settings import settings  # noqa: E402

EXPERIMENT_NAME = "hkia_baseline"
RUN_NAMES = ["run 1", "run 2", "run 3", "run-4", "run-5"]
...
def _resolve_tracking_uri() -> str:
    ...
    if sqlite_db.exists():
        # MLflow accepts forward slashes on Windows for sqlite URIs.
...
def main() -> None:
    ...
    for run_name in RUN_NAMES:
        ...
        n = len(traces_df)
        total_traces += n
        print(f"  {run_name!r} (run_id={run_id}) -> {out_path.relative_to(PROJECT_ROOT)} ({n} traces)")
